...
def test_import_paths():
    ...
    try:
        ...
        assert 'from .agent import create_graph' in content, "agent_wrapper.py 导入路径不正确"
...
def test_file_content():
    ...
    try:
        ...
        assert 'def create_graph(' in content
        print("✓ create_graph 函数存在")
        
        assert 'def call_model(' in content
        print("✓ call_model 函数存在")
        
        assert 'def run_tools(' in content
        print("✓ run_tools 函数存在")
        
        assert 'def should_continue(' in content
        print("✓ should_continue 函数存在")
        
        assert 'workflow = StateGraph(MessagesState)' in content
...
def test_agent_wrapper_functionality():
    ...
    try:
        ...
        # 检查图创建逻辑
        assert 'self.assistant_ui_graph = create_graph(self.llm, self.task_tools)' in content
